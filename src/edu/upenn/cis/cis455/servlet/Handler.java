...
public class Handler extends DefaultHandler {
		...
		public void characters(char[] ch, int start, int length) {
			...
			if (m_state == 1 || m_state == 6) {
				m_servletName = value;
				System.out.println("servletName:" + m_servletName + "\nstart:" + start + "\nlen:" + length);
				m_state = (m_state == 1)? 0: 6;
			} else if (m_state == 2) {
				m_servlets.put(m_servletName, value);
				System.out.println("servlets:" + m_servlets.toString() + "\nstart:" + start + "\nlen:" + length);
				m_state = 0;
				...
				System.out.println("urlPattern:" + m_urlPattern + "\nm_state:" + m_state);
				m_state = 0;
			} else if (m_state == 11) {
				if (m_paramName == null) {
				...
				System.out.println("contextParams:" + m_contextParams + "\nparamName:" + m_paramName+ "\nstart:" + start + "\nlen:" + length);
				m_paramName = null;
				m_state = 0;
			} else if (m_state == 21) {
				if (m_paramName == null) {
